...
file_type : IDENTIFIER {
  if (_strcmpi($1, "csv") == 0) {
    $$ = kImportCSV;
  } else if (_strcmpi($1, "tbl") == 0) {
    $$ = kImportTbl;
  } else if (_strcmpi($1, "binary") == 0 || _strcmpi($1, "bin") == 0) {
    $$ = kImportBinary;
  } else {
    free($1);
...
| operand '~' operand { $$ = Expr::makeOpBinary($1, kOpPosix, $3); }
...
| table_ref_atomic opt_join_type JOIN table_ref_atomic USING '(' column_name ')' {
  ...
  if ($7->alias != nullptr) left_col->alias = _strdup($7->alias);
  if ($1->getName() != nullptr) left_col->table = _strdup($1->getName());
  auto right_col = Expr::makeColumnRef(_strdup($7->name));
  if ($7->alias != nullptr) right_col->alias = _strdup($7->alias);
  if ($4->getName() != nullptr) right_col->table = _strdup($4->getName());
